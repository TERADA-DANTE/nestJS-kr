...
NestJS는 Node.js 서버 사이드 애플리케이션을 구축하기 위한 **프레임워크**입니다. 사실상 스탠드얼론 API서버를 위한 프레임워크라고 생각해도 무방합니다.
...
그것보다 제일 난감한 것은 개발자마다 다른 디자인 패턴이죠. 어느정도 룰이 있으면 좋으련만... 이라는 생각을 줄곧 해왔습니다.

한편, 좋아하는 프레임워크 중 하나인 Next.js 는 API기능을 가지고 있지만, 사실 이건 정적 페이지 생성 프레임워크에 포함되어있는 **덤**이라고 생각하고 있습니다. 유효성 검증이나, 변환 등의 지원도 미미하고, 응답이나 예외처리 부분에서도 상당히 손이 많이갑니다. 버전 12정도 되면 API지원도 괜찮아질까요?
