...
exception 개념안에는 `@useFilter()` 를 사용하는 filter의 개념도 있는데, filter는 다양한 인풋에 대해서 로그를 남기거나, 전혀다른 JSON 스키마를 응답하는 등의 exception layer 전반의 풀 컨트롤을 제공합니다. 이것에 관해서는 추후에 정리하겠습니다.# 시작하기에 앞서
